"""Chinese-to-target-language subtitle translation (C1).

Consumes ``subtitles_zh.json`` (B2 output) and produces
``uploads/<job_id>/subtitles_<target_lang>.json`` plus downloadable reference
files ``subtitles_<target_lang>.srt`` and ``subtitles_<target_lang>_plain.txt``
(the per-language filenames come from ``lang_files``, F12b).
...
the original Chinese text in the translated-text field with
``translation_fallback: true`` — silent mismatches are never allowed, and a
whole batch is no longer thrown away just because one line is problematic.
The recursion is bounded by ``max_split_rounds`` so pathological input falls
back gracefully instead of looping; an exhausted shared ``CallBudget`` keeps
what already matched and falls the rest back without raising.

The translation prompt names the target language from
``job_config.TARGET_LANG_NAMES[target_lang]`` (F12f). The translated text is
written under ``text_translated`` (``lang_files.TRANSLATED_TEXT_FIELD``),
which readers look up through ``lang_files.entry_text`` with a fallback to the
pre-F12f ``text_hi`` field.
...
TRANSLATION_PROMPT = (
    ...
    "natural {language}. Reply with EXACTLY {count} lines, one translation per "
...
RETRY_PROMPT = (
    ...
    "into natural {language}. You MUST reply with EXACTLY {count} non-empty "
...
def build_srt(entries):
    ...
    for entry in entries:
        text = lang_files.entry_text(entry) or entry.get("text_zh") or ""
        blocks.append(
            f"{entry['serial']}\n"
...
def build_plain(entries):
    """Build a plain text file: serial + translated text, one per line."""
    return "\n".join(
        f"{entry['serial']}\t{lang_files.entry_text(entry) or ''}" for entry in entries
    ) + "\n"
    ...
    for entry in entries:
        ...
        if not text_zh:
            ...
            if candidate is None:
                ...
                text, is_fallback = candidate, False
        output.append(
            {
                "serial": serial,
                "text_zh": text_zh,
                lang_files.TRANSLATED_TEXT_FIELD: text,
...
def translate_subtitles(
    ...
    language = job_config.TARGET_LANG_NAMES.get(lang, "Hindi")
